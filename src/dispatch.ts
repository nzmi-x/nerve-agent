// The permission gate (D4). PLAN is read-only; YOLO runs everything. The mode is passed in from the
...
// Deliberately conservative: anything not here is refused in PLAN — build a tool or switch to YOLO.
...
export function planBashAllowed(command: string): Decision {
  ...
  return SAFE_PROGRAMS.has(prog)
    ? { ok: true }
    : { ok: false, reason: `'${prog}' isn't an obviously-safe read-only command — build a tool, or switch to YOLO` };
}
...
export function allowed(tool: Tool, args: Record<string, unknown>, mode: Mode): Decision {
  ...
  return { ok: false, reason: `${tool.name} mutates — blocked in PLAN mode (switch to YOLO to run it)` };
}
