...
import { task } from "./task.ts"; // imports `tools` back from here, but only reads it at runtime (safe cycle)
...
const TOOL_MODULES: { path: string; name: string }[] = [
  ...
  { path: "./search.ts", name: "search" },
  { path: "./task.ts", name: "task" },
];

/** The active tool set. `let`, not `const`, so `reloadTools` can swap it. */
export let tools: Tool[] = [read, write, edit, bash, ls, grep, glob, manual, askUser, lsp, notebook, todo, fetchTool, search, task];
